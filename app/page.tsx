      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:my-10 place-content-center">
        ...
        <div className="rounded-md p-6 w-full bg-light-grey dark:bg-dark-grey transition-all duration-200 ease-in-out drop-shadow-lg">
          ...
          <div className="mt-2">
            ...
            {Resume.skills.programmingLanguages.map((val, i) => (<span key={i} className="rounded-lg bg-light-teal dark:bg-dark-teal drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">Libraries & Frameworks</p>
            {Resume.skills.librariesFrameworks.map((val, i) => (<span key={i} className="rounded-lg bg-light-purple dark:bg-dark-purple drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">Databases</p>
            {Resume.skills.databases.map((val, i) => (<span key={i} className="rounded-lg bg-light-orange dark:bg-dark-orange drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">CI/CD</p>
            {Resume.skills.ciCd.map((val, i) => (<span key={i} className="rounded-lg bg-light-teal dark:bg-dark-teal drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">Cloud Computing</p>
            {Resume.skills.cloudComputing.map((val, i) => (<span key={i} className="rounded-lg bg-light-purple dark:bg-dark-purple drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">Tools</p>
            {Resume.skills.tools.map((val, i) => (<span key={i} className="rounded-lg bg-light-orange dark:bg-dark-orange drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
          <div className="mt-2">
            <p className="font-light py-0.5">Product Management</p>
            {Resume.skills.pm.map((val, i) => (<span key={i} className="rounded-lg bg-light-teal dark:bg-dark-teal drop-shadow-md font-bold py-1 px-2 m-1 inline-block">{val}</span>))}
          </div>
        </div>
